...
for model_idx in range(len(depths)):
    results_txt_path = os.path.join("MIMOConv/plots/results_txt/symbols", f"WRN-{depths[model_idx]}-{widths[model_idx]}.txt")
    ...
    ax1.bar(time_bars, COMM_symbols, width=bar_width, color='C4', hatch=hatch_style[8], edgecolor="black", linewidth=2, alpha=0.9)
    ...
    ax1.set_ylabel('Total transmitted\nI/Q symbols', fontsize=fontsize)
    ...
    ax1.tick_params(axis='both', which='both', labelsize=fontsize-10, size=5)
